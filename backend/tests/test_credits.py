...
@pytest.mark.asyncio
async def test_check_and_reserve_sufficient(db_session: AsyncSession, test_user: User):
    service = CreditService(db_session)
    result = await service.check_and_reserve(test_user.id, 5)
    assert result is True


@pytest.mark.asyncio
async def test_check_and_reserve_insufficient(db_session: AsyncSession, test_user: User):
    service = CreditService(db_session)
    result = await service.check_and_reserve(test_user.id, 999)
...
async def test_deduct_credits(db_session: AsyncSession, test_user: User):
    service = CreditService(db_session)
    result = await service.deduct(test_user.id, 3, "generation")
...
async def test_deduct_insufficient(db_session: AsyncSession, test_user: User):
    service = CreditService(db_session)
    result = await service.deduct(test_user.id, 999, "generation")
    assert result is False
...
async def test_refund_credits(db_session: AsyncSession, test_user: User):
    service = CreditService(db_session)
    await service.deduct(test_user.id, 5, "generation")
...
async def test_transaction_history(db_session: AsyncSession, test_user: User):
    service = CreditService(db_session)
    await service.deduct(test_user.id, 1, "generation")
    await service.deduct(test_user.id, 2, "generation")
    await service.add_credits(test_user.id, 50, "purchase")
