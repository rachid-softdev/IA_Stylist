...
Start-Process -FilePath cmd -ArgumentList "/c","pnpm dev (depuis apps/web)" 
  -RedirectStandardOutput "$env:TEMP\qa\IA_Stylist.dev.txt" 
  -RedirectStandardError "$env:TEMP\qa\IA_Stylist.err.txt" -WindowStyle Hidden
...
`
apps/web/scripts/qa\        # (Ã  crÃ©er si campagne complÃ¨te)
